...
class Pinyin:
    ...
    data_path = os.path.split(
        os.path.realpath(sys.argv[0]))[0]+'\Mandarin.dat'

    def __init__(self, data_path: str = str(data_path)) -> None:
        lines = Path(data_path).read_text().splitlines()
        self.pinyins = dict(tuple(line.split('\t', maxsplit=1))
                            for line in lines)
                    ...
                    if tone != 0:
                        ...
                        if m is None:
                            ...
                            for num, vowels in enumerate(("a", "o", "e", "ui", "iu")):
                                if vowels in t:
                                    t = t.replace(
                                        vowels[-1], PinyinToneMark[tone][num])
                                    break
                r += t
                t = ""
        r += t
    ...
    def get_pinyins(self, chars: str, splitter: str = '-',
        ...
        """
        all_pinyin_options = [
        ]  # a list of lists that we'll fill with all pinyin options for each character
        # in the list (otherwise, probably not a Chinese character)
        is_in_list = True
        for char in chars:
            key = f"{ord(char):X}"
            if key not in self.pinyins:
                if is_in_list:
                    ...
                    # add to previous sequence of non Chinese chars
                    all_pinyin_options[-1][-1] += char
            else:
                if tone_marks is None:  # in this case we may have duplicates if the variations differ just by the tones
                    ...
                    for v in self.pinyins[key].split():
                        # we remove the tone mark while we're at it
                        if v[0:-1] not in char_py_options:
                            char_py_options.append(v[0:-1])
                else:
                    char_py_options = self.pinyins[key].split()
                last = 1 if n == 1 else len(char_py_options)
                if tone_marks == 'marks':
                    char_options = [Pinyin.decode_pinyin(
                        o) for o in char_py_options[0:last]]
                else:  # 'numbers' or None
                    char_options = [o for o in char_py_options[0:last]]

                all_pinyin_options.append(
                    [Pinyin.convert_pinyin(c, convert) for c in char_options])
                is_in_list = True
